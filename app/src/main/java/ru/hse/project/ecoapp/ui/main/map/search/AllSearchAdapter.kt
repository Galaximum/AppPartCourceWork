...
class AllSearchAdapter() : RecyclerSwipeAdapter<SearchViewHolder>(), Filterable {
    ...
    override fun onBindViewHolder(holder: SearchViewHolder, position: Int) {
        ...
        holder.setOnClickHiddenBtnListener {
            clickHiddenBtnListener!!.invoke(it).addOnSuccessListener {
                filter.filteredData.elementAt(position).isFavorite =
                    !filter.filteredData.elementAt(position).isFavorite
    ...
    fun setOnClickListener(it: (Double, Double) -> Unit) {
        clickListener = it
    }
